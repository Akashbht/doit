...
class ChordClient:
    def __init__(self, port_number=5000):
        ...
        self.data = {}   # data attribute for the chord client 
        self.create
    ...
    def periodic_stabilization(self):
        while True:
            ...
            time.sleep(0.1)  # timely invoke stabilization routines every 100 ms
    ...
    def update_finger_table(self, ip, port):
        ...
        # finding the node on first finger which is greater than or equal to new node

        for i in range (1, len(self.finger_table)):
            finger_start = (int(self.node_id, 16) + 2** (i-1) % (2**160))

            if finger_start > int(new_node_id,16):
    ...
    def join(self, ip, port):
        ...
        response = self.send_request(ip,port,"join request",node_id)
    ...
    def send_request(self, ip, port, request_type, data):
    ...
    def put(self, key, value):
        ...
        successor_client = successor[1]
 
        if isinstance(successor_client, ChordClient):
            successor_client.put_locally(key, value)
            
        else:
            print("Error: successor_client is not a ChordClient object.")
            self.data[key] = value
    ...
    def closest_preceding_node(self, key):
        ...
        for i in range(len(self.finger_table) - 1, 0, -1):
            if self.finger_table[i] and isinstance(self.finger_table[i][1], ChordClient) and self.finger_table[i][1].node_id < key:
    ...
    def stabilize(self):
        ...
        if self.successor_list:  # Check if the successor list is not empty
            # Get the successor's predecessor
            if isinstance(self.successor_list[0][1], ChordClient):
                successor_predecessor = self.successor_list[0][1].predecessor
            ...
            if isinstance(self.successor_list[0][1], ChordClient):
                self.successor_list[0][1].notify(self)
        # else:
