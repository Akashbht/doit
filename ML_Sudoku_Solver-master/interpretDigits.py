...
images, labels = next(iter(dataiterator))
...
number_images = []
for filename in os.listdir('D:\\TCSOOPs\\akash_pycharm\\PROJECTS\\ML_Sudoku_Solver-master\\NumberCells\\cell'):
    img = cv2.imread(os.path.join('D:\\TCSOOPs\\akash_pycharm\\PROJECTS\\ML_Sudoku_Solver-master\\NumberCells\\cell', filename))
    if img is not None:
...
for i in range(len(number_images)):
    ...
    number_images[i] = cv2.adaptiveThreshold(number_images[i], 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
    # Resize to 28x28 pixels
    number_images[i] = cv2.resize(number_images[i], (28, 28))
    number_images[i] = number_images[i].astype('float32')
...
numbers = []
for img in number_images:
    features = torch.from_numpy(img).reshape(1, 1, 28, 28)  # Reshape for Torch
    with torch.no_grad():
        logps = model(features)
    ps = torch.exp(logps)
    probab = ps.numpy()[0]
    prediction = np.argmax(probab)
    print("Predicted Digit =", prediction, "  Probability =", probab[prediction])
    numbers.append(prediction)
...
puzzle_file = open("D:\\TCSOOPs\\akash_pycharm\\PROJECTS\\ML_Sudoku_Solver-master\\Puzzle.txt", 'r')
...
puzzle_file = open("D:\\TCSOOPs\\akash_pycharm\\PROJECTS\\ML_Sudoku_Solver-master\\Puzzle.txt", 'w')
